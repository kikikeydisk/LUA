...
local SCRIPT_FILE_ADDR = "https://raw.githubusercontent.com/kikikeydisk/LUA/main/zuoci.lua?token=ARUE3537AXM64Z34QUNRTTDBZ3GTY";
local VERSION_FILE_ADDR = "https://raw.githubusercontent.com/kikikeydisk/LUA/main/version.txt?token=ARUE35YN77AYOFTDIYYSKITBZ3G4A";
...
callbacks.Register( "Draw", "handleUpdates", function()
	if updateframes < 5.5 then
		...
		draw.Text(7 - 650 + fadein, 7, "ZuoCi's");
		draw.Color(225,225,225,255 - fadeout);
		draw.Text(7 + draw.GetTextSize("ZuoCi's ") - 650 + fadein, 7, "Script");
		draw.Color(255,150,75,255 - fadeout);
		draw.Text(7 + draw.GetTextSize("ZuoCi's Script  ") - 650 + fadein, 7, "\\");
		spacing = draw.GetTextSize("ZuoCi's Script  \\  ");
...
local tab = gui.Tab(rb_ref, "ZuoCi", ("ZuoCi's Script " .. VERSION_NUMBER));
...
local function Regular()
	...
	if kek == 1 then
		...
		if drawRight == 0 and drawLeft == 0 and drawBack == 0 then
			local customyaw = gui.GetValue("rbot.ZuoCi.yawangle");
			local customjitter = gui.GetValue("rbot.ZuoCi.jitteramount");
